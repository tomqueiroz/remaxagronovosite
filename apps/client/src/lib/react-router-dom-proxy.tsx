...
export function collectRoutePathsForMessaging(children: AnyEl) {
  return Array.from(flattenRoutes(children)).sort();
}

function postAllRoutesOnce(children: AnyEl) {
  if (routesPosted) return;
  try {
    const list = collectRoutePathsForMessaging(children);
